...
solve xs = length $ filter (\(x, y) -> x == y) $ zip intersectLengths smallestLengths
  where
    intersectLengths = map length intersects
    intersects = map (\(x, y) -> intersect x y) sections
    smallestLengths = map (\(x, y) -> min (length x) (length y)) sections
